...
> **Trạng thái**: OLD (Hết hạn) — Đã được thay thế bởi [10_BRD_export_order_management.md v1.3](../../BRD/10_BRD_export_order_management.md)
> **Ngày**: 2026-03-10 (cập nhật lần cuối: 2026-03-12)
> **Tham chiếu**: BRD Xếp Xe (`08_BRD_manifest_management.md`), BRD Mã Hàng (`07_BRD_product_code_management.md`)
>
> **Lịch sử thay đổi**:
> - v1.0 (draft này): 1 lệnh = 1 khách hàng
> - v1.1: Bỏ ràng buộc 1 khách hàng (cho phép nhiều khách)
> - **v1.2**: Khôi phục ràng buộc 1 khách hàng. Thêm validate FE + filter khách trong popup chọn mã hàng (Luồng B)
> - **v1.3 (hiện tại)**: Thêm tính năng thanh toán giao hàng — đổi "chi phí giao hàng" → "phí ship" (nhập tại bước giao, không nhập khi tạo lệnh), hiển thị "Tổng tiền khách phải trả" = chi phí NK + phí ship, thêm checkbox "Đã nhận tiền" (`paymentReceived`). Không nhận thiếu — nếu không nhận = công nợ.
...
| `deliveryCost` | Int? | VND | Phí ship — nhập tại bước giao hàng (không nhập khi tạo lệnh) |
| `notes` | String? | Text Area | Ghi chú |
| `status` | `ExportOrderStatus` | Enum | Trạng thái lệnh xuất kho (xem mục 2.3) |
| `paymentReceived` | Boolean | Default `false` | Nhân viên kho xác nhận đã nhận đủ tiền. `false` = công nợ (implement sau) |
...
3. Nhân viên mở ExportOrder → màn hình giao hàng hiển thị:
   - Chi phí NK hàng hóa (tổng totalImportCostToCustomer) — chỉ đọc
   - Nhập: Phí ship (deliveryCost)
   - Hiển thị: Tổng tiền khách phải trả = Chi phí NK + Phí ship (real-time)
   - Tick: "Đã nhận tiền" (paymentReceived)
     + true = đã nhận đủ
     + false = khách nợ → công nợ (implement sau)
4. Bấm [Xác nhận xuất kho]
5. Backend: ExportOrder.status = DA_XUAT_KHO
           + ExportOrder.deliveryCost = phí ship đã nhập
...
| 4 | Mỗi ExportOrder chỉ được tạo cho 1 khách hàng | Đảm bảo logic xuất kho theo từng khách, tránh nhầm lẫn khi giao hàng và tính toán công nợ. Khôi phục lại ở v1.2 sau khi bỏ ở v1.1. |
